...
class ButterBean_Control {
	...
	public function to_json() {
        ...
        $this->json['default_option']     = $this->default_option;
		$this->json['active']      = $this->is_active();

		$this->json['value']       = isset( $this->settings[ $this->setting ] ) ? $this->get_value( $this->setting )      : '';

        if(empty($this->json['value']) and $this->default_option and butterbean()->is_new_post) {
            $options = get_option('stm_theme_options');
            if(!empty($options[$this->name])) {
                $default_option = $options[$this->name];
                if($default_option == 'false') $default_option = '';
                $this->json['value'] = $default_option;
            }
        }
