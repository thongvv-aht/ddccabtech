...
class ButterBean_Control_File extends ButterBean_Control {
    ...
    public function __construct( $manager, $name, $args = array() ) {
        ...
        $this->l10n = wp_parse_args(
            $this->l10n,
            array(
                'upload'      => esc_html__( 'Add pdf',         'butterbean' ),
                'set'         => esc_html__( 'Set as pdf',      'butterbean' ),
                'choose'      => esc_html__( 'Choose pdf',      'butterbean' ),
                'change'      => esc_html__( 'Change pdf',      'butterbean' ),
                'remove'      => esc_html__( 'Remove pdf',      'butterbean' ),
                'placeholder' => esc_html__( 'No pdf selected', 'butterbean' )
            )
        );
    }
