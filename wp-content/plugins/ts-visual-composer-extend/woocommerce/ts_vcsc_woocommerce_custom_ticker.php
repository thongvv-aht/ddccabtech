		...
		class TS_WooCommerce_Ticker_Basic {
							...
							$output .= '<div class="ts-newsticker-elements-frame ' . $newsticker_elements . ' ' . $ticker_border_radius . '" style="">';
								...
								$output .= '<ul id="ts-newsticker-ticker-' . $woo_random . '" class="newsticker ' . $ticker_border_radius . '" style="' . $newsticker_header . '">';
									while ($loop->have_posts()) : $loop->the_post();
										$postCounter++;
										$postAttributes = 'data-full="' . get_post_time($date_format) . '" data-time="' . get_post_time($time_format) . '" data-author="' . get_the_author() . '" data-date="' . get_post_time('U') . '" data-modified="' . get_the_modified_time('U') . '" data-title="' . the_title_attribute('echo=0') . '" data-comments="' . get_comments_number() . '" data-id="' .  get_the_ID() . '"';
										...
										$output .= '<li ' . $postAttributes . '>';
											...
											if (($ticker_price == "true") && ($variable == "false")) {
												$output .= '<span class="ts-newsticker-pricetag" style="">';
													if ($costs_current > 0) {
														if ($costs_current && isset($costs_regular)) {
															...
															$to 	= $costs_current;
															if ($from != $to) {
																$output .= '<span class="ts-newsticker-product-regular" style="float: none;"><del>' . ((is_numeric($from)) ? wc_price($from) : $from) . '</del> | </span><span class="ts-newsticker-product-special" style="float: none;">' . ((is_numeric($to)) ? wc_price($to) : $to) . '</span>';
															} else {
																$output .= '<span class="ts-newsticker-product-current" style="float: none;">' . ((is_numeric($to)) ? wc_price($to) : $to) . '</span>';
															}
														} else {
															$to = $costs_current;
															$output .= '<span class="ts-newsticker-product-current" style="float: none;">' . ((is_numeric($to)) ? wc_price($to) : $to) . '</span>';
												...
												$output .= '<span class="ts-newsticker-pricetag" style="">';
													$output .= '<span class="ts-newsticker-product-current" style="float: none;">' . ((is_numeric(min($prices))) ? wc_price(min($prices)) : min($prices)) . ' - ' . ((is_numeric(max($prices))) ? wc_price(max($prices)) : max($prices)) . '</span>';
												$output .= '</span>';		
											}											
											// Sale Ribbon
											if ($onsale == "true") {
			...
			function TS_VCSC_WooCommerce_Ticker_Basic_Elements() {
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					"name"                              => __( "TS Basic Products Ticker", "ts_visual_composer_extend" ),
