<?php
	...
	function TS_VCSC_Image_Switch_Function ($atts, $content = null) {
		...
		if (($dynamic_front == "true") && ($dynamic_start != "")) {
			$image_start					= "";
			$dynamic_start					= rawurldecode(base64_decode(strip_tags($dynamic_start)));
			$dynamic_start					= do_shortcode($dynamic_start);
			$dynamic_start					= (int)$dynamic_start;
			if (($dynamic_start!= "") && (is_numeric($dynamic_start))) {
				$image_start				= $dynamic_start;
			}
		}		
		if (($dynamic_back == "true") && ($dynamic_end != "")) {
			$image_end						= "";
			$dynamic_end					= rawurldecode(base64_decode(strip_tags($dynamic_end)));
			$dynamic_end					= do_shortcode($dynamic_end);
			$dynamic_end					= (int)$dynamic_end;
			if (($dynamic_end!= "") && (is_numeric($dynamic_end))) {
				$image_end					= $dynamic_end;
			}
		}
		...
		$nacho_data							= 'rel="' . ($lightbox_group == "true" ? "krautgroup" : $lightbox_group_name) . '" data-share="' . ($lightbox_social == "true" ? 1 : 0) . '" data-effect="' . $lightbox_effect . '" data-duration="' . $lightbox_speed . '" ' . $nacho_color . '';
		...
		if (($switch_event == "link") && ($switch_link != '')){
			...
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-media nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="' . $switch_image_start[0] . '" target="_blank" title="' . $alt_attribute_start . '" data-thumbnail="' . $switch_image_start[0] . '" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';
			$linkswitch_modal				= '';
		} else if ($switch_event == "back") {
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-media nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="' . $switch_image_end[0] . '" target="_blank" title="' . $alt_attribute_end . '" data-thumbnail="' . $switch_image_end[0] . '" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';			
			$linkswitch_modal				= '';
		} else if ($switch_event == "both") {
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-media nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="' . $switch_image_start[0] . '" target="_blank" title="' . $alt_attribute_start . '" data-thumbnail="' . $switch_image_start[0] . '" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';
			$linkswitch_end					.= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-media nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0; display: none;" href="' . $switch_image_end[0] . '" target="_blank" title="' . $alt_attribute_end . '" data-thumbnail="' . $switch_image_end[0] . '" ' . $nacho_data . ' data-random="' . $randomizer . '">' . $alt_attribute_end . '</a>';
			$linkswitch_modal				= '';
		} else if ($switch_event == "other") {
			$linkswitch_image 				= wp_get_attachment_image_src($lightbox_image, 'full');
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-media nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="' . $linkswitch_image[0] . '" target="_blank" title="' . $lightbox_title . '" data-thumbnail="' . $linkswitch_image[0] . '" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';
			$linkswitch_modal				= '';
		} else if ($switch_event == "modal") {
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' kraut-lightbox-modal nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="#' . $switch_image_id . '-modal" target="_blank" title="' . $alt_attribute_start . '" data-type="html" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';
			// Create hidden DIV with Modal Content
			$linkswitch_modal = '<div id="' . $switch_image_id . '-modal" class="ts-modal-content kraut-lb-hide-if-javascript ' . $el_class . '" style="display: none; padding: 20px;">';
