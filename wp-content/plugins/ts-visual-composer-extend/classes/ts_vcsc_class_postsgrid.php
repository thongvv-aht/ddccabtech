									...
									<?php while ($isoposts->have_posts() ) :
										...
										if ($matched_terms == 0) {									
											...
											if ($postCounter < $posts_limit + 1) {
															...
															$output .= '<div class="ts-timeline-column" style="margin: 0;">';
																$output .= '<div class="ts-timeline-text-wrap" style="">';
																	$output .= '<div class="ts-timeline-text-wrap-inner" style="width: 100%; left: 0;">';
																		$output .= '<' . $title_wrap . ' class="ts-timeline-title" style="text-align: center; margin: 0; padding: 0;">' . $postPeriod . '</' . $title_wrap . '>';
																	$output .= '</div>';
												...
												$postAttributes = 'data-visible="false" data-full="' . get_post_time($date_format) . '" data-author="' . get_the_author() . '" data-date="' . get_post_time('U') . '" data-modified="' . get_the_modified_time('U') . '" data-title="' . $postTitleAttribute . '" data-comments="' . get_comments_number() . '" data-id="' .  get_the_ID() . '"';
											?>
															...
															if ($show_featured == "true") {
																if ((strlen(get_the_post_thumbnail()) > 0) && (strlen(get_post_thumbnail_id()) > 0)) {
																	...
																	<div class="ts-isotope-posts-thumb">
																		<a href="<?php the_permalink() ?>" target="' . $content_target . '"><img src="<?php echo $thumbnail[0]; ?>" alt=""></a>
																	</div>
																<?php }
															}
														?>
														<<?php echo $title_wrap; ?> class="ts-isotope-posts-title" data-title="<?php echo urldecode($postTitleAttribute); ?>"><a href="<?php the_permalink() ?>" target="<?php echo $content_target; ?>"><?php the_title(); ?></a></<?php echo $title_wrap; ?>>
