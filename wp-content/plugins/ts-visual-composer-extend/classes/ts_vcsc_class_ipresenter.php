			...
			function TS_VCSC_iPresenter_Item ($atts, $content = null) {
				...
				$output .= '<div class="ts-ipresenter-step clearFixMe" ' . $animation . ' data-thumbnail="' . $thumbnail_image[0] . '">';
					if (($lightbox_image == "true") && ($frontend_edit == "false")) {
						$output .= '<a href="' . $ipresenter_image[0] . '" class="ts-ipresenter-lightbox kraut-lightbox-media no-ajaxy" data-title="' . $title . '" rel="' . ($lightbox_group == "true" ? "krautgroup" : $lightbox_group_name) . '" data-effect="' . $lightbox_effect . '" data-duration="' . $lightbox_speed . '" ' . $nacho_color . '>';
					}
						$output .= '<img class="ts-ipresenter-image" src="' . $ipresenter_image[0] . '" data-no-lazy="1" />';
					if (($lightbox_image == "true") && ($frontend_edit == "false")) {
						$output .= '</a>';
					}
					$output .= '<' . $title_wrap . ' class="ts-ipresenter-title clearFixMe" style="color: ' . $title_color . ';">' . $title . '</' . $title_wrap . '>';
