										...
										while ($isoposts->have_posts() ) :
											...
											if ($matched_terms == 0) {
												...
												if ($postCounter < $posts_limit + 1) {
													...
													$postAttributes = 'data-visible="false" data-full="' . get_post_time($date_format) . '" data-author="' . get_the_author() . '" data-date="' . get_post_time('U') . '" data-modified="' . get_the_modified_time('U') . '" data-title="' . urldecode($postTitleAttribute) . '" data-comments="' . get_comments_number() . '" data-id="' .  get_the_ID() . '"';
												?>
														...
														<div class="ts-timeline-column">
															<div class="ts-timeline-text-wrap ts-timeline-text-wrap-date" style="position: relative;">
																<?php
																	...
																	echo '<' . $title_wrap . ' class="ts-isotope-posts-title" data-title="' . urldecode($postTitleAttribute) . '"><a href="' . get_permalink() . '" target="' . $content_target . '">' . the_title('', '', false) . '</a></' . $title_wrap . '>';
																?>
																...
																<?php
																	if ($show_share == 'true') {
																		echo '<div class="ts-isotope-posts-social">';
																			echo '<a href="http://pinterest.com/pin/create/link/?url=' . get_permalink() . '&amp;description=' . $postTitleAttribute . '" class="ts-isotope-posts-social-holder" rel="external" target="_blank"><span class="ts-isotope-posts-social-pinterest"></span></a>';
																			echo '<a href="https://plusone.google.com/_/+1/confirm?hl=en&amp;url=' . get_permalink() . '&amp;name=' . $postTitleAttribute . '" class="ts-isotope-posts-social-holder" rel="external" target="_blank"><span class="ts-isotope-posts-social-google"></span></a>';
																			echo '<a href="http://twitter.com/share?text=' . $postTitleAttribute . '&url=' . get_permalink() . '" class="ts-isotope-posts-social-holder" rel="external" target="_blank"><span class="ts-isotope-posts-social-twitter"></span></a>';
																...
																<?php if (($show_categories == 'true') || ($show_tags == 'true')) {
																	echo '<div class="ts-isotope-posts-metadata clearFixMe">';
																			...
																			<div class="ts-isotope-posts-taxonomies">
																				<?php
																					if ($post_type == 'post') {
																						...
																						$categories 	= get_the_terms(get_the_ID(), $post_taxos);
																					}
																					$separator 			= ' / ';
																					$output 			= '';
																					if ($categories){
																						...
																						foreach($categories as $category) {
																							$output .= '<a href="' . get_category_link($category->term_id) . '" class="ts-isotope-posts-categories" title="' . esc_attr( sprintf( __( "View all posts in %s" ), $category->name ) ) . '">' . ($post_type == 'post' ? $category->cat_name : $category->name) . '</a>' . $separator;
																						}
																		...
																		<?php }
																		// Post Tags
																		if (($show_tags == 'true') && ($post_type == 'post')) { ?>
																	...
																	<?php
																		...
																		if ($show_featured == "true") {
																			if ('' != get_the_post_thumbnail()) { 
																				$thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'); ?>
																				<div class="ts-isotope-posts-thumb">
																					<a href="<?php the_permalink() ?>" target="' . $content_target . '"><img src="<?php echo $thumbnail[0]; ?>"></a>
																				</div>
																			<?php }
																		}
																		echo '<' . $title_wrap . ' class="ts-isotope-posts-title" data-title="' . urldecode($postTitleAttribute) . '"><a href="' . get_permalink() . '" target="' . $content_target . '">' . the_title('', '', false) . '</a></' . $title_wrap . '>';
			...
			function TS_VCSC_Add_Posts_Slider_Elements() {
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					...
					"params"                            => array(
						...
						array(
							"type"              	    => "switch_button",
							"heading"                   => __( "Loop Content", "ts_visual_composer_extend" ),
							"param_name"                => "items_loop",
							"value"                     => "false",
							"description"               => __( "Switch the toggle if you want to provide an endless loop for the slider content.", "ts_visual_composer_extend" ),
							"dependency" 				=> array("element" 	=> "slider_type", "value" 	=> "owlslider"),
							"group" 			        => "Slider Settings",
						),
						array(
